...
public class FastLinearCrypto implements Destroyable{
	...
	private byte[][] rotors;
	private int[] rotorPos1;
	private int[] rotorPos2;
	...
	public FastLinearCrypto(byte[] key) throws IllegalArgumentException{
		...
		rotorPos1 = new int[ROTOR_NUMBER];
		rotorPos2 = new int[ROTOR_NUMBER];
		rotors = new byte[ROTOR_NUMBER][0];
		rotors[0] = generateRoundKey(roundKeys[ROUNDS-1]);
		rotors[0] = generateRoundKey(rotors[0]);
		for (int i = 1; i < rotors.length; i++) {
			rotors[i] = generateRoundKey(rotors[i-1]);
		}
	...
	public byte[] encrypt(byte[] b, int saltPos){
		for (int i = 0; i < rotorPos1.length; i++) {
			rotorPos1[i] = 0;
			rotorPos2[i] = 0;
		}
		...
		size++;
		size *= wordSize;
		if(size-b.length >= wordSize+PADDIN_MIN_LENGHT){
			size-=wordSize;
		}
		...
		for (int i = 0; i < size; i++) {
			if(i<k-2){
				...
				int u = sr.nextInt(256*256/wordSize);
				u = (k-PADDIN_MIN_LENGHT)+u*(wordSize);
				r[i] = (byte)(u%256);
				r[i+1] = (byte)(u/256);
				//System.out.println("FLC: E"+(k)+" ("+u+" "+(r[i]&0xff)+" "+(r[i+1]&0xff)+")");
				break;
			}
		}
		if(num <= 1){
			return encryptBlock(r, saltPos);
		}
		int lastPos = saltPos;
		for (int i = 0; i < num; i++) {
			byte[] tr = Arrays.copyOfRange(r, i*wordSize, (i+1)*wordSize);
			int nextPos = getSum(1, tr);
			tr = encryptBlock(tr, lastPos);
			lastPos += nextPos;
			if(lastPos>=100000)
				lastPos -= 100000;
			for (int j = 0; j < tr.length; j++) {
	...
	public byte[] decrypt(byte[] b, int saltPos){
		for (int i = 0; i < rotorPos1.length; i++) {
			rotorPos1[i] = 0;
			rotorPos2[i] = 0;
		}
		...
		for (int i = 0; i < r.length; i+=wordSize) {
			byte[] tr = decryptBlock(Arrays.copyOfRange(b, i, i+wordSize), pos);
			pos += getSum(1, tr);
			if(pos>=100000)
				pos -= 100000;
			for (int j = 0; j < tr.length; j++) {
		...
		int paddingEnd = ((r[r.length-2] & 0xff)+(r[r.length-1] & 0xff)*256)%(wordSize);
		paddingEnd += PADDIN_MIN_LENGHT;
		
		//System.out.println("FLC: D"+paddingEnd+" ("+(r[r.length-1] & 0xff)+")");
		return Arrays.copyOfRange(r, paddingEnd-2, r.length-2);
	}
	
	private byte[] encryptBlock(byte[] b, int k){
		int i = 0;
		for (; i < ROUNDS/2-1; i++) {
	...
	private byte[] decryptBlock(byte[] b, int k){
		int i = ROUNDS-1;
		for (; i >= ROUNDS-2; i--) {
			b = DEC_singleRound(b, i, k);
		}
		xOR(b, saltTable, k%223);
		superMixup(b, false, true);
		int pos = getSum(7, lineMixupTable)+getSum(k%3, roundKeys[1]);
		DEC_doLMT(b, pos+k/7);
		for (; i >= ROUNDS/2-1; i--) {
			b = DEC_singleRound(b, i, k);
		}
		superMixup(b, false, false);
		xOR(b, saltTable, k/3+getSum(3, roundKeys[0]));
	...
	private int latSumSuperMixup(boolean t){
		int u = 0;
		for (int i = 0; i < rotorPos1.length; i++) {
			if(t)
				u += (int)(rotors[i][rotorPos1[i]] & 0xff);
			else
				u += (int)(rotors[i][rotorPos2[i]] & 0xff);
		}
		return u;
	}
	
	private void superMixupDone(byte u, boolean t){
		if(t){
			rotorPos1[(int)(u & 0xff)%ROTOR_NUMBER]++;
			if(rotorPos1[(int)(u & 0xff)%ROTOR_NUMBER] >= wordSize)
				rotorPos1[(int)(u & 0xff)%ROTOR_NUMBER] = 0;
		}else{
			rotorPos2[(int)(u & 0xff)%ROTOR_NUMBER]++;
			if(rotorPos2[(int)(u & 0xff)%ROTOR_NUMBER] >= wordSize)
				rotorPos2[(int)(u & 0xff)%ROTOR_NUMBER] = 0;
		}
	}
	
	private void superMixup(byte[] q, boolean enc, boolean t){
		if(!enc){
			for (int i = 0; i < q.length; i++) {
				q[i] -= latSumSuperMixup(t);
				superMixupDone(q[i], t);
			}
		}else{
			byte u;
			for (int i = 0; i < q.length; i++) {
	...
	public static byte[] generateRoundKey(byte[] pre){
