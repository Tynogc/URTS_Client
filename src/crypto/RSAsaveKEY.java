...
public final class RSAsaveKEY implements Destroyable{
	...
	/*public PrivateKey getPrivateKey()throws IllegalStateException{
		...
		if(encrypted)throw new IllegalStateException("Key is Encrypted");
	...
	/**
	 * Locks the Key by encrypting the Private Exponent
	 */
	public final void lock(RSAsaveKEY key){
		if(destroyed)return;
		if(encrypted)return;
		encrypted = true;
		
		lockedExponent = RSAcrypto.encryptByte(privateExponent.toByteArray(), key, true);
		//Destroy Private Exponent
		privateExponent.xor(new BigInteger(privateExponent.bitCount(), new Random()));
		privateExponent = new BigInteger("12345");
		privateExponent = null;
	}
	...
	/**
	 * Undo the Encryption of the private Exponent
	 */
	public final void unlock(RSAsaveKEY key) throws Exception{
		if(!encrypted)throw new IllegalStateException("This Key isn't locked...");
		
		try {
			privateExponent = new BigInteger(RSAcrypto.decryptByte(lockedExponent, key, false));
		} catch (Exception e) {
			throw new SecurityException("Decryption Failed: Unlock-Key");
		}
		encrypted = false;
		if(!runTest()){
			encrypted = true;
			privateExponent = null;
			throw new SecurityException("Decryption Failed: Unlock-Key-Test");
		}
	}
	...
	private static void testEncryption(RSAsaveKEY key) throws Exception{
		String plaintext = crypto.Random.generateRandomString(34+(int)(Math.random()*40));

		System.out.println("Decypher: "+plaintext+" "+plaintext.getBytes().length);
		// Encrypt digest
		byte[] cipherText = RSAcrypto.encryptByte(plaintext, key, true);
		
		byte[] decypher = RSAcrypto.decryptByte(cipherText, key, false);
