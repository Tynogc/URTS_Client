...
public class RSAcrypto {
	...
	private static final int PADDING_FOUL_ROUDS = 5;
	 ...
	 * [9 rndm. bytes upperPadding] [1byte upper Padding size] [upperPadding]
	...
	public static byte[] addPadding(byte[] b, int bitLenght) throws SecurityException{
		...
		int higherPadding;
		if(toFill>256)
			higherPadding = asr.nextInt(241)+15;//TODO check for problems
		else
		...
		//Over-Padding: Hashes the bytes 0 to higherPadding and uses it, to "encrypt" the message
		...
		//Let us be sure, there is no Pattern left: Foul-Padding-Scematic
		int middle = bNew.length/2;
		int max = bNew.length;

		//Takes the first half (as hash) and adds it to the last half or vice versa
		for (int u = 0; u < PADDING_FOUL_ROUDS; u++) {
			messageDigest.reset();
			int a1; //Some Position information for different Rounds...
			int a2;
			int a3;
			int a4;
			if(u%3 == 0){
			...
			//Digest a Hash from [a1 to a2[ and XOrs it to [a3 to a4[
			messageDigest.update(bNew, a1, a2-a1);
			byte[] q = messageDigest.digest();
			
			int p = 0;
			for (int i = a3; i < a4; i++) {
				bNew[i] = (byte)(bNew[i] ^ q[p]);
				p++;
				if(p>=q.length)
					p = 0;
			}
	...
	private static byte[] removePaddingIntern(byte[] b) throws SecurityException{
		// The Padding was Fould... Removeing...
		int middle = b.length/2;
		int max = b.length;
		//TODO own Algorithm if necessary
		MessageDigest messageDigest;
		try {
			messageDigest = MessageDigest.getInstance("SHA-256");
		} catch (NoSuchAlgorithmException e) {
		...
		//Takes the first half (as hash) and adds it to the last half or vice versa
		for (int u = PADDING_FOUL_ROUDS-1; u >= 0; u--) {
			int a1; //Some Position information for different Rounds...
			int a2;
			int a3;
			int a4;
			...
			//Digest a Hash from [a1 to a2[ and XOrs it to [a3 to a4[
			messageDigest.update(b, a1, a2-a1);
			byte[] q = messageDigest.digest();
